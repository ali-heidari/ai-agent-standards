...
## Sync instructions — command definition

- Trigger: the user writes "sync instructions" (case-insensitive, anywhere in their message).
- Purpose: re-enforce the base rules from this standards repository onto the current project without requiring the user to specify what to update.
- Required steps (execute in order, no skipping):
  1. Read all six base convention files from this repo:
  ...
  2. Locate the project's instruction file: `.agent/agent-instructions.md` at the project root.
     - If the `.agent/` folder does not exist, create it.
     - If `agent-instructions.md` does not exist inside it, create it from scratch.
  3. Merge base rules into `.agent/agent-instructions.md`:
     - Preserve any project-specific sections that do not conflict.
     - Overwrite or add any section that is missing or contradicts the canonical base rules.
     - Do not leave duplicate or conflicting entries.
  4. Copy the six convention files into `.agent/base/` (create folder if needed), overwriting stale versions.
  5. Report to the user which files were created, updated, or unchanged.
- After sync, the project MUST satisfy these invariants:
  - `.agent/agent-instructions.md` exists and contains all core workflow rules from `assistant-conventions.md`.
  - `.agent/base/` contains all six convention files at their canonical content.
  - No rule in any `.agent/` file contradicts the base standard (project additions are allowed; overrides are not).
